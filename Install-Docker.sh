...
WORKDIR /root/TRSS_Amiya'>Dockerfile
...
docker run -itd -h TRSS-Amiya --name $DKNAME -v "$DIR":/root/TRSS_Amiya --restart always $([ $DKNAME = TRSS_Amiya ]&&echo "-p 8088:8088"||echo "-p 8088") trss:amiya||abort "Docker 容器启动失败"
mkdir -vp "$CMDPATH"&&
