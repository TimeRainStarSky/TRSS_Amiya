...
download(){ case "$N" in
  ...
  4)SERVER="GitLab";URL="https://gitlab.com/TimeRainStarSky/TRSS_Amiya/raw/main";;
  5)SERVER="Coding";URL="https://trss.coding.net/p/TRSS/d/Amiya/git/raw/main";;
  6)SERVER="Aliyun";URL="https://code.aliyun.com/TimeRainStarSky/TRSS_Amiya/raw/main";;
  7)SERVER="GitCode";URL="https://gitcode.net/TimeRainStarSky1/TRSS_Amiya/raw/main";;
  8)SERVER="JiHuLab";URL="https://jihulab.com/TimeRainStarSky/TRSS_Amiya/raw/main";;
  9)SERVER="Bitbucket";URL="https://bitbucket.org/TimeRainStarSky/TRSS_Amiya/raw/main";;
  10)SERVER="Jsdelivr";URL="https://cdn.jsdelivr.net/gh/TimeRainStarSky/TRSS_Amiya@main"
esac
echo "
...
RUN pacman-key --init &&\
    ...
    rm -rf /var/cache
RUN echo -n '\''bash /root/TRSS_Amiya/Main.sh "$@"'\''>/usr/local/bin/tsai &&\
    chmod 755 /usr/local/bin/tsai &&\
    pacman -Syu --noconfirm --needed --overwrite "*" jre-openjdk unzip &&\
    rm -rf /var/cache
...
$Y- 正在启动 Docker 容器$O
"
docker run -itPd -h TRSS-Amiya --name TRSS_Amiya -v "$DIR":/root/TRSS_Amiya --restart=always trss:amiya||abort "Docker 容器启动失败，若要重装容器，请先停止并删除已安装容器"
echo -n "docker exec -it TRSS_Amiya bash /root/TRSS_Amiya/Main.sh "'"$@"'>"$CMDPATH/$CMD"&&chmod 755 "$CMDPATH/$CMD"||abort "脚本执行命令 $CMDPATH/$CMD 设置失败，手动执行命令：docker exec -it TRSS_Amiya bash /root/TRSS_Amiya/Main.sh"
echo "
